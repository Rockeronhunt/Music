...
# musicoin_hash
RUN git clone https://github.com/musicoinpay/musicoin_hash
RUN cd musicoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} musicoin
RUN useradd -u ${USER_ID} -g musicoin -s /bin/bash -m -d /musicoin musicoin
...
RUN mkdir /musicoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /musicoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /musicoin-src
