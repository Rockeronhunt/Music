...
// which are part of the MUSICOIN repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="musicoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /musicoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "musicoinpay-musicoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from musicoinpay-musicoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'musicoinpay-musicoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from musicoinpay-musicoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /musicoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /musicoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /musicoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /musicoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /musicoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /musicoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /musicoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
