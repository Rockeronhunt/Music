Cross-compiliation of MUSICOIN Core
===============================

MUSICOIN Core can be cross-compiled on Linux to all other supported host systems. This is done by changing
the `HOST` parameter when building the dependencies and then specifying another `--prefix` directory when building MUSICOIN.
...
When building MUSICOIN Core, use
