...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all MUSICOIN related functionality
...
/*
    ...
    allow endless transaction that would bloat MUSICOIN and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all MUSICOIN specific functionality
