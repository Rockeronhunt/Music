...
const char * const BITCOIN_CONF_FILENAME = "musicoin.conf";
const char * const BITCOIN_PID_FILENAME = "musicoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "musicoin" is a composite category enabling all MUSICOIN-related debug output
                if(ptrCategory->count(std::string("musicoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\MUSICOIN
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\MUSICOIN
    // Mac: ~/Library/Application Support/MUSICOIN
    // Unix: ~/.musicoin
#ifdef WIN32
    // Windows
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/MUSICOIN";
#else
    // Unix
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty musicoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override musicoin.conf
