...
{
    return QCoreApplication::translate("musicoin-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in musicoin.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in musicoin.qrc)
...
/** Class encapsulating MUSICOIN Core startup and shutdown.
...
/** Main MUSICOIN application object */
...
void BitcoinApplication::initializeResult(bool success)
{
    ...
    if(success)
    {
        ...
        // musicoin: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. MUSICOIN Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(musicoin);
    Q_INIT_RESOURCE(musicoin_locale);
    ...
    /// 6. Determine availability of data directory and parse musicoin.conf
    ...
    // musicoin: links repeatedly have their payment requests routed to this process:
