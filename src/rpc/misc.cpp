...
UniValue getinfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total musicoin balance of the wallet\n"
            "  \"privatesend_balance\": xxxxxx, (numeric) the anonymized musicoin balance of the wallet\n"
...
UniValue debug(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "                       mempoolrej, net, proxy, prune, qt, rand, reindex, rpc, selectcoins, tor, zmq, musicoin\n"
            ...
            "\nExamples:\n"
            + HelpExampleCli("debug", "musicoin")
            + HelpExampleRpc("debug", "musicoin+net")
        );
...
UniValue validateaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nReturn information about the given musicoin address.\n"
            "\nArguments:\n"
            "1. \"address\"     (string, required) The musicoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"address\", (string) The musicoin address validated\n"
...
UniValue createmultisig(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 2)
    {
        std::string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are musicoin addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) musicoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 3)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"         (string, required) The musicoin address to use for the signature.\n"
...
{ //  category              name                      actor (function)         okSafeMode
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    /* MUSICOIN features */
    { "musicoin",               "mnsync",                 &mnsync,                 true,  {} },
    { "musicoin",               "spork",                  &spork,                  true,  {"value"} },
